...
class TestRegistration:
    ...
    def test_registration_user_success(self, driver):
    ...
    def test_registration_user_invalid_email_error(self, driver):
        driver.find_element(*Main.BUTTON_LOGIN_REGISTRATION).click()
        WebDriverWait(driver, 5).until(
            expected_conditions.element_to_be_clickable(Registration.BUTTON_NO_ACCOUNT)
        )
        ...
        assert (
            ...
            and driver.find_element(
                *Registration.FIELD_REGISTRATION_REPEAT_PASSWORD_ERROR
            )
            and driver.find_element(*Registration.ERROR_EMAIL)
        )

    def test_registration_existing_user__error(self, driver):
        driver.find_element(*Main.BUTTON_LOGIN_REGISTRATION).click()
        WebDriverWait(driver, 5).until(
            expected_conditions.element_to_be_clickable(Registration.BUTTON_NO_ACCOUNT)
        )
